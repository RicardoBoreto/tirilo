...
class ControladorOlhos:
    def __init__(self, arquivo_config=None):
        ...
        self.estado = {
            "olho_direito":  {"h": 50.0, "v": 50.0, "pc": 0.0, "pb": 0.0, "boca": 0.0},
            "olho_esquerdo": {"h": 50.0, "v": 50.0, "pc": 0.0, "pb": 0.0}
        }
    ...
    def mover_suave(self, olho, h_alvo=None, v_alvo=None, p_alvo=None, pc_alvo=None, pb_alvo=None, duracao=0.5):
    ...
    def mover_suave_ambos(self, h_alvo=None, v_alvo=None, p_alvo=None, pc_alvo=None, pb_alvo=None, duracao=0.5):
        def t_dir(): self.mover_suave("olho_direito", h_alvo, v_alvo, p_alvo, pc_alvo, pb_alvo, duracao)
        def t_esq(): self.mover_suave("olho_esquerdo", h_alvo, v_alvo, p_alvo, pc_alvo, pb_alvo, duracao)
    ...
    def olhar_neutro(self, suave=True):
        if suave: self.mover_suave_ambos(50, 50, 20, 0.4)
        else:
    ...
    def abrir_olhos(self):
        self.mover_suave_ambos(50, 50, 0, 0.3)
    ...
    def surpresa(self):
        self.mover_suave_ambos(p_alvo=-10, duracao=0.2)

    def olhar_triste(self):
        self.mover_suave_ambos(50, 20, 70, 0.8)

    def olhar_feliz(self):
        self.mover_suave_ambos(50, 80, 40, 0.6)

    def olhar_cima(self, suave=True):
        """Olha para o alto (V=10) subindo apenas a pálpebra inferior (PB=50)."""
        if suave: self.mover_suave_ambos(h_alvo=50, v_alvo=10, pc_alvo=0, pb_alvo=50, duracao=0.7)
        else:
    ...
    def olhar_frente(self, suave=True):
        """Olha para frente (posição neutra)."""
        if suave: self.mover_suave_ambos(50, 50, 0, duracao=0.5)
        else: self.olhar_para(50, 50)

    def desconfiado(self):
        self.mover_suave_ambos(80, 50, 70, 0.5)
